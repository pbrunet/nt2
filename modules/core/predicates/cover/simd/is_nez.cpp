...
NT2_TEST_CASE_TPL ( is_nez_real__1_0,  NT2_SIMD_REAL_TYPES)
{
  using nt2::is_nez;
    ...
    for(nt2::uint32_t j = 0; j < NR;j+=cardinal_of<n_t>::value)
      {
        ...
        for(nt2::uint32_t i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_EQUAL( v[i],ssr_t(nt2::is_nez (a0[i])));
        }
      }

  }
} // end of test for floating_
NT2_TEST_CASE_TPL ( is_nez_int__1_0,  NT2_SIMD_INTEGRAL_TYPES)
{
  using nt2::is_nez;
  using nt2::tag::is_nez_;
  using nt2::load;
