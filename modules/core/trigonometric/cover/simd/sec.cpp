...
//==============================================================================
#define NT2_UNIT_MODULE "nt2 trigonometric toolbox - sec/simd Mode"
...
NT2_TEST_CASE_TPL ( sec_real__1_0,  NT2_SIMD_REAL_TYPES)
{
  using nt2::sec;
  using nt2::tag::sec_;
  using nt2::load;
  ...
  typedef typename nt2::meta::call<sec_(T)>::type sr_t;
    ...
    for(nt2::uint32_t j = 0; j < NR;j+=cardinal_of<n_t>::value)
      {
        ...
        for(nt2::uint32_t i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_ULP_EQUAL( v[i],ssr_t(nt2::sec (a0[i])), 1.0);
...
NT2_TEST_CASE_TPL ( sec_int_convert__1_0,  NT2_SIMD_INT_CONVERT_TYPES)
{
  using nt2::sec;
  using nt2::tag::sec_;
  using nt2::load;
    ...
    for(nt2::uint32_t j = 0; j < NR;j+=cardinal_of<n_t>::value)
      {
        ...
        for(nt2::uint32_t i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_ULP_EQUAL( v[i], nt2::sec (a0[i]), 1.0);
          ulp0 = nt2::max(ulpd,ulp0);
        }
      }
    std::cout << "max ulp found is: " << ulp0 << std::endl;
...
NT2_TEST_CASE_TPL ( sec_uint_convert__1_0,  NT2_SIMD_UINT_CONVERT_TYPES)
{
  using nt2::sec;
  using nt2::tag::sec_;
  using nt2::load;
