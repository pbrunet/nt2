...
//==============================================================================
#define NT2_UNIT_MODULE "nt2 trigonometric toolbox - inrad/simd Mode"
...
NT2_TEST_CASE_TPL ( inrad_real__1_0,  NT2_SIMD_REAL_TYPES)
{
  using nt2::inrad;
  ...
  typedef typename nt2::meta::call<inrad_(vT)>::type r_t;
  typedef typename nt2::meta::call<inrad_(T)>::type sr_t;
    ...
    for(nt2::uint32_t j = 0; j < NR;j+=cardinal_of<n_t>::value)
      {
        ...
        for(nt2::uint32_t i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_ULP_EQUAL( v[i],ssr_t(nt2::inrad (a0[i])), 1.0);
...
NT2_TEST_CASE_TPL ( inrad_uint_convert__1_0,  NT2_SIMD_UINT_CONVERT_TYPES)
{
  ...
  static const nt2::uint32_t NR = NT2_NB_RANDOM_TEST;
  {
    NT2_CREATE_BUF(tab_a0,T, NR, T(0), T(2000));
    double ulp0, ulpd ; ulpd=ulp0=0.0;
    for(nt2::uint32_t j = 0; j < NR;j+=cardinal_of<n_t>::value)
      {
        vT a0 = load<vT>(&tab_a0[0],j);
