...
NT2_TEST_CASE_TPL ( multiplies_real__2_0,  NT2_SIMD_REAL_TYPES)
{
    ...
    for(nt2::uint32_t j = 0; j < NR;j+=cardinal_of<n_t>::value)
      {
        ...
        r_t v = multiplies(a0,a1);
        for(nt2::uint32_t i = 0; i< cardinal_of<n_t>::value; i++)
        {

          NT2_TEST_EQUAL( v[i],ssr_t(nt2::multiplies (a0[i],a1[i])));
        }
      }
...
NT2_TEST_CASE_TPL ( multiplies_integer__2_0,  NT2_SIMD_INTEGRAL_TYPES)
{
  using nt2::multiplies;
  using nt2::tag::multiplies_;
  using nt2::load;
